...
final class SubscriberRegistry {
    ...
    void register(Object listener) {
        Multimap<Class<?>, Subscriber> listenerMethods =
            findAllSubscribers(listener);//获取对应的监听方法，可能有多个
        ...
        for (Map.Entry<Class<?>, Collection<Subscriber>> entry : listenerMethods.asMap().entrySet()) {
            ...
            CopyOnWriteArraySet<Subscriber> eventSubscribers =
                subscribers.get(eventType);//获取已有的订阅方法

            if (eventSubscribers == null) {//不包含任何订阅方法
    ...
    Iterator<Subscriber> getSubscribers(Object event) {
        ...
        //需要获取所有父级别类，因为父类也是可以接受到消息的。如Integer类型的事件，Number类订阅方法也是可以接收到消息的。
    ...
    /**
     * 获取该监听器内定义的所有的事件处理方法。
     * 以事件为key，事件订阅者为value。根据标记类型创建同步或异步订阅者。
     * 由于listener内可能有多个监听方法具有相同的参数，因此采用MultiMap。采用Map<Class, List<subscriber>>>会不会更好？
     * <p/>
     * </>
    ...
    private Multimap<Class<?>, Subscriber> findAllSubscribers(Object listener) {
        ...
        for (Method method : getAnnotatedMethods(clazz)) {//获取所有有subscribe注解的方法
            Class<?>[] parameterTypes = method.getParameterTypes();
            Class<?> eventType = parameterTypes[0];//只看第一个参数，因此不支持多个参数的事件
            methodsInListener.put(eventType, Subscriber.create(bus, listener, method));//对该类型事件创建对应的订阅者事件。
        }
